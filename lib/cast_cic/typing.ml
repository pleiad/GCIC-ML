...
let rec infer_type (ctx : context) (t : term) : (term, [> type_error ]) result =
  match t with
  | Var id ->
    (try Ok (NameMap.find id ctx) with
  ...
  | Prod { id; dom; body } ->
    ...
    Ok (Universe (Kernel.Variants.product_universe_level i j))
  | Unknown ty ->
...
and check_type (ctx : context) (t : term) (ty : term) : (unit, [> type_error ]) result =
  let* ty' = infer_type ctx t in
  are_convertible ty ty'

and infer_prod (ctx : context) (t : term) : (Name.t * term * term, [> type_error ]) result
  =
  let* ty = infer_type ctx t in
  let* v = reduce ty in
  match v with
