...
let reduce1 (term, cont) : state =
  ...
  | Unknown (Prod fi), _ ->
      ...
      (* Prod-Err *)
  | Err (Prod fi), _ ->
      ( Lambda { fi with body = Err fi.body }, cont )
      (* Down-Unk *)
...
let fill_hole1 term = function
  ...
  | KErr -> Err term
  | KCast_source (target, term') ->
      Cast { source = term; target; term = term' }
  | KCast_target (source, term') ->
      Cast { source; target = term; term = term' }
  | KCast_term (source, target) ->
