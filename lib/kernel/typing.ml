...
let rec infering (ctx : context) (t : term) : (term, type_error) result =
  ...
  | App (t, u) ->
     let* (id, dom, cod) = infering_prod ctx t in
     let* () = checking ctx u dom in
     Ok (subst id u cod)
  | Lambda {id; dom; body} ->
     let* _ = infering_univ ctx dom in
     let* cod = infering (Context.add ~key:id ~value:dom ctx) body in
     Ok (Prod {id; dom; cod})
  | Prod {id; dom; cod} -> 
     let* j = infering_univ ctx dom in
     let* i = infering_univ (Context.add ~key:id ~value:dom ctx) cod in
