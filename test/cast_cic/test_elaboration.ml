...
let tests_base_elaborations () =
  ...
  Alcotest.(check (pair Testable.term Testable.term))
    ...
    (Elaboration.elaborate empty_ctx (Lambda { id; dom = Unknown 1; body = Var id })
    |> Result.get_ok);
  Alcotest.(check (pair Testable.term Testable.term))
    "Prod with universe elaborates"
    (Prod { id; dom = Universe 1; body = Var id }, Universe 2)
    (Elaboration.elaborate empty_ctx (Prod { id; dom = Universe 1; body = Var id })
    |> Result.get_ok);
  Alcotest.(check (pair Testable.term Testable.term))
    ...
    (let dom =
     ...
     , Universe 0 ))
    (Elaboration.elaborate empty_ctx (Prod { id; dom = Unknown 1; body = Var id })
    |> Result.get_ok);
  Alcotest.(check (pair Testable.term Testable.term))
    "app without unknown"
    ( App
  ...
  Alcotest.(check (pair Testable.term Testable.term))
    ...
    , unknown 0 )
    (Elaboration.elaborate empty_ctx (App (Unknown 1, Universe 0)) |> Result.get_ok)
