...
let tests_base_elaborations () =
  ...
  Alcotest.(check (pair Testable.term Testable.term))
    ...
    (Elaboration.elaborate empty_ctx idf |> Result.get_ok);
  Alcotest.(check (pair Testable.term Testable.term))
    "lambda with unknown elaborates"
    (let dom =
       Ast.Cast
         { source = unknown 1; target = Universe 0; term = Unknown (unknown 1) }
     in
     (Lambda { id; dom; body = Var id }, Prod { id; dom; body = dom }))
    (Elaboration.elaborate empty_ctx
       (Lambda { id; dom = Unknown 1; body = Var id })
    |> Result.get_ok);
  Alcotest.(check (pair Testable.term Testable.term))
    "Prod with universe elaborates"
    (Prod { id; dom = Universe 1; body = Var id }, Universe 2)
    (Elaboration.elaborate empty_ctx
       (Prod { id; dom = Universe 1; body = Var id })
    |> Result.get_ok);
  Alcotest.(check (pair Testable.term Testable.term))
     ...
     ( Prod
         {
           id;
           dom;
           body = Cast { source = dom; target = Universe (-1); term = Var id };
         },
       Universe 0 ))
    (Elaboration.elaborate empty_ctx
       (Prod { id; dom = Unknown 1; body = Var id })
    |> Result.get_ok);
  Alcotest.(check (pair Testable.term Testable.term))
    "app without unknown"
    ( App
      ...
      Universe 1 )
    (Elaboration.elaborate empty_ctx (App (idf, Universe 0)) |> Result.get_ok);
  Alcotest.(check (pair Testable.term Testable.term))
    "app with unknown"
    ( App
        ( Cast
            {
              source = unknown 1;
              target = Ast.germ 1 HProd;
              term = Unknown (unknown 1);
            },
          Cast { source = Universe 1; target = unknown 0; term = Universe 0 } ),
      unknown 0 )
    (Elaboration.elaborate empty_ctx (App (Unknown 1, Universe 0))
    |> Result.get_ok)
      ...
      Prod
        {
          ...
          body = unknown (Ast.cast_universe_level 1);
        } )
...
let correct_elaboration =
  ...
  QCheck.(
    Test.make ~count:1000 ~name:"correct elaboration" Arbitrary.gcic_term
      (fun t ->
        let elab_term = Elaboration.elaborate NameMap.empty t in
        assume (elab_term |> Result.is_ok);
        let t', ty = Result.get_ok elab_term in
        match Typing.infer_type NameMap.empty t' with
