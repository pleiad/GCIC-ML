...
let test_lookup () =
  Alcotest.(check (option int)) "lookup singleton" 
  (Some 1)
  (lookup ~key:"x" ~ctx:example1);
  Alcotest.(check (option int)) "lookup empty" 
  None
  (lookup ~key:"x" ~ctx:empty);
  Alcotest.(check (option int)) "lookup missing" 
  None
  (lookup ~key:"y" ~ctx:example1)


let test_remove () =
  Alcotest.(check bool) "remove empty ctx" 
  true
  (is_empty (remove ~key:"x" ~ctx:empty));
  Alcotest.(check bool) "remove at head" 
  true
  (is_empty (remove ~key:"x" ~ctx:example1));
  Alcotest.(check (option int)) "remove in between" 
  None
  (lookup ~key:"x" ~ctx:(remove ~key:"x" ~ctx:(add ~key:"y" ~value:2 example1)))

let test_to_string () = 
  let string_of_key k = k in 
  let string_of_value = string_of_int in 
  let to_str = to_string string_of_key string_of_value in
  Alcotest.(check string) "to_string empty ctx" 
  "[]"
  (to_str empty);
  Alcotest.(check string) "to_string non-empty ctx" 
  "(y, 2) ; (x, 1) ; []"
