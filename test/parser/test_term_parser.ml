...
let tests_application () =
  Alcotest.check pterm "nullary application" (Ok (Universe 0)) (parse_term "Type@0");
  Alcotest.check
    pterm
    "unary application"
...
let tests_let () =
  Alcotest.check
    ...
    (parse_term "let x : Type@0 := ?0 in x");
...
let tests_ascription () =
  ...
  Alcotest.(check bool)
    ...
    true
    (parse_term "let x : ?0 : ?1 := Type@0 in x" |> Result.is_error);
  Alcotest.(check bool)
    "ascription in let binding body fails"
    true
    (parse_term "let x : ?0 = Type@0 : Type@1 in x" |> Result.is_error)

let tests_unicode () =
  Alcotest.check pterm "universe" (Ok (Universe 0)) (parse_term "□");
  Alcotest.check
    pterm
    "lambda"
