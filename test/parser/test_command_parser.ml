...
let tests_eval () =
  Alcotest.check pcommand "Succeds" (Ok (Eval (Universe 0)))
    (parse_command "eval Type 0;;");
  Alcotest.(check bool)
    "Fails with :" true
    (parse_command "eval Type 0 : ?0;;" |> Result.is_error);
  Alcotest.(check bool)
    "Fails if no expression" true
    (parse_command "eval;;" |> Result.is_error);
  Alcotest.(check bool)
    "Fails if no semi-colons" true
    (parse_command "eval ?0 foo" |> Result.is_error)

let tests_check () =
  Alcotest.check pcommand "check command"
...
let tests_elaborate () =
  Alcotest.check pcommand "elab command" (Ok (Elab (Universe 0)))
